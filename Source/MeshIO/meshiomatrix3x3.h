#ifndef _MESH_MESHMATRIX3X3_
#define _MESH_MESHMATRIX3X3_
...
struct meshio::Matrix3x3
{
  ...
  {
    double i_vec[] = {vec.xComp,vec.yComp,vec.zComp};
  ...
  {
    vals[0 + 3*i] = vec.xComp;
    vals[1 + 3*i] = vec.yComp;
    vals[2 + 3*i] = vec.zComp;
  }

  /**Set column j using a vector.*/
  void SetColJVec(int j, Vector vec)
  {
    vals[j + 3*0] = vec.xComp;
    vals[j + 3*1] = vec.yComp;
    vals[j + 3*2] = vec.zComp;
  }
